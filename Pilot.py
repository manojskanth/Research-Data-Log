...
# Parent Folder ID (The 2025-26 Central Hub)
...
# Note: ID 1aAP24... is confirmed as English & Languages sitting in the Languages drive
DEPT_FOLDERS_25_26 = {
...
def upload_to_drive(file_bytes, filename, folder_id, mime_type):
    ...
    # Use BytesIO to create a fresh stream for every upload call
...
if submit and f_file:
    try:
        with st.spinner(f"Routing data to {f_dept}..."):
            ...
            # Prepare file data once as a variable
            ext = f_file.name.split('.')[-1]
            filename = f"{f_name}_{f_activity}_{datetime.datetime.now().strftime('%H%M')}.{ext}"
            file_data = f_file.getvalue() # Read the content into memory
            mtype = f_file.type

            # SYNC 1: Upload to Child Department Folder (Languages Drive)
            # This is the primary target that was failing.
            doc_link = upload_to_drive(file_data, filename, DEPT_FOLDERS_25_26[f_dept], mtype)
            
            # SYNC 2: Upload to Central Hub Backup (2025-26 Parent)
            upload_to_drive(file_data, filename, RESEARCH_MASTER_ID, mtype)
            
            # SYNC 3: Faculty G-Site Portfolio Folder
            ...
            st.success(f"✅ Record successfully synced for {f_name} to {f_dept}")
            st.balloons()
