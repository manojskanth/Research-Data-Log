...
# --- 2. THE UPLOAD ENGINE (QUOTA-SAFE) ---
def get_creds():
...
def upload_to_drive(file_bytes, filename, folder_id, mime_type):
    """Uploads a file using the Folder Owner's quota."""
    ...
    # Refresh memory stream for every sync destination
    media = MediaIoBaseUpload(io.BytesIO(file_bytes), mimetype=mime_type, resumable=True)
    
    # supportsAllDrives=True is essential for routing to different departmental drives
...
if submit and f_file:
    try:
        with st.spinner(f"Routing record for {f_name}..."):
