...
# The 2025-26 Parent Folder (Central Hub)
RESEARCH_MASTER_ID = "1c67i0HzSRxpMMrmwwBtpK5A-RpaGcVZY"

# Child Department Folders
# UPDATED: Mapping the new ID you provided for English & Languages
DEPT_FOLDERS_25_26 = {
...
def upload_to_drive(file_bytes, filename, folder_id, mime_type):
    ...
    # Create a fresh stream for every call to ensure data isn't 'lost' between uploads
...
if submit and f_file:
    try:
        with st.spinner(f"Depositing record in {f_dept}..."):
            ...
            # SYNC 1: DIRECT DEPOSIT IN DEPARTMENT FOLDER (High Priority)
            # This is the folder accessible by the department
            target_folder = DEPT_FOLDERS_25_26[f_dept]
            doc_link = upload_to_drive(file_data, filename, target_folder, mtype)
            
            # SYNC 2: CENTRAL HUB BACKUP (2025-26 Parent)
            ...
            st.success(f"✅ Records successfully deposited in {f_dept} and Central Hub.")
            st.balloons()
